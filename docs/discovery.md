...
| Date | ID | Decision |
|---|---|---|
| 2026-08-19 | — | **Docs strategy: hexdocs stays, plus a separate static site on GitHub Pages.** Hexdocs remains the canonical API reference (published as usual on `mix hex.publish` — it's where Elixir developers look first, and ex_doc already gates `mix check`). On top of it: hand-built static artifacts on GitHub Pages, themed like the Wiretap UI (phosphor green), for higher-production engineering docs — the showcase the stock ExDoc look can't be. The two cross-link. Pipeline decided same day: GitHub Actions deployment (`upload-pages-artifact` → `deploy-pages`, Pages source = Actions), no custom domain — default `curtisault.github.io/wiretap/`. Guides inventory settles at the v1.0 discovery session. Details: [production/discovery.md](production/discovery.md) |
